...
const HISTORY_LEN = 200; // ~2,5 часа при шаге 30 c
...
function generateController(ctrl) {
  ...
  return {
    ...
    history: generateMockHistory(200),
...
function generateMockHistory(count = 2000) {
  const now = Date.now();
  const step = 30 * 1000; // 30 секунд, как в реальном обновлении
  ...
  for (let i = count - 1; i >= 0; i--) {
    ...
    times.push(ts);
    temp.push(randomBetween(10, 30));
    rh.push(randomBetween(40, 80));
    nh3.push(Math.round(randomBetween(0, 50)));
  }

  return { times, temperature: temp, humidity: rh, nh3 };
}
...
function MetricChart({ data, times, min, max }) {
  ...
  ? Math.min(window.innerWidth - 80, 360)
  : 260;
  const height = 80;
  const count = data.length;
  const stepX = count > 1 ? width / (count - 1) : width;
...
function ChannelRow({ ch, onVfdClick }) {
  ...
  return (
    <div className="mb-1 text-xs">
        <div
          className="flex flex-col sm:flex-row sm:items-center sm:justify-between cursor-pointer select-none"
        ...
        <span
          className={`mt-1 sm:mt-0 px-2 py-0.5 rounded-full text-[10px] font-semibold ${
...
function MetricDashboardModal({ selection, controllers, onClose }) {
  ...
  if (times.length > 0) {
    ...
    const stepMs = RANGE_STEPS[rangeId] || HISTORY_SAMPLE_INTERVAL_MS;
...
// downsampling by step
let last = 0;
const picked = [];
for (const p of filtered) {
  if (picked.length === 0 || p.ts - picked[last].ts >= stepMs) {
    picked.push(p);
    last = picked.length - 1;
  }
}

slicedData = picked.map(p => data[p.i]);
slicedTimes = picked.map(p => times[p.i]);
// если слишком мало точек — показываем всю историю, а не пустой график
if (slicedData.length < 2) {
  ...
  if (slicedData.length > 0) {
    statMin = Math.min(...slicedData).toFixed(1);
    statMax = Math.max(...slicedData).toFixed(1);
    statAvg = (slicedData.reduce((a,b)=>a+b,0) / slicedData.length).toFixed(1);
  }
      ...
      <div className="bg-white rounded-xl shadow-lg p-4 w-full max-w-xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="mb-2 flex items-center justify-between gap-2">
          <div className="text-sm text-gray-600">
            ...
            <span>
              min: <span className="font-semibold">{statMin}</span> {config.unit}
            </span>
            <span>
              avg: <span className="font-semibold">{statAvg}</span> {config.unit}
            </span>
            <span>
              max: <span className="font-semibold">{statMax}</span> {config.unit}
            </span>
          </div>
          </div>
...
function VfdDashboardModal({ selection, controllers, onClose }) {
  ...
  const renderCurrentGF = () => (
    <table className="w-full text-[11px] break-words">
      <tbody>
        <tr>
  ...
  const renderFaultGroup = (title, baseCode, f) => {
    ...
    return (
      <div className="mb-1">
        ...
        <table className="ww-full text-[11px] break-words">
          <tbody>
            <tr>
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl shadow-lg p-4 w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-3">
          <div>
            <div className="text-xs text-gray-500">{controller.title}</div>
          ...
          <button
            className="text-xs px-3 py-2 rounded border border-gray-300 hover:bg-gray-100 active:bg-gray-200"
            onClick={onClose}
          >
            Закрыть
          </button>
...
export default function App() {
  ...
  const [vfdFaultHistory, setVfdFaultHistory] = useState([]);
    ...
    // добавление/удаление мок данных вручную
    function modifyHistory(ctrls, add = true, count = 100) {
      return ctrls.map(c => {
        const h = c.history;
        if (add) {
          for (let i = 0; i < count; i++) {
            const ts = new Date(Date.now() - (count - i) * 30000).toISOString();
            h.temperature.push(randomBetween(10, 30));
            h.humidity.push(randomBetween(40, 80));
            h.nh3.push(Math.round(randomBetween(0, 50)));
            h.times.push(ts);
          }
        } else {
          h.temperature.splice(0, count);
          h.humidity.splice(0, count);
          h.nh3.splice(0, count);
          h.times.splice(0, count);
        }
        return { ...c, history: h };
      });
    }
  
    // загрузка реальных данных из БД (позже)
    ...
    <div className="min-h-screen bg-gray-100 p-6">
      <header className="mb-6 flex items-center justify-between">
        <div>
          ...
          <button
  className="text-xs px-3 py-1.5 ml-3 rounded bg-blue-600 text-white hover:bg-blue-700 active:bg-blue-800"
  onClick={() => {
    setControllers(prev => modifyHistory(prev, !mockMode));
    setMockMode(v => !v);
  }}
>
  {mockMode ? "Убрать данные" : "Добавить данные"}
</button>
        ...
        <span className="text-xs text-gray-500">
        {controllers.length} контроллеров • {mockMode ? "mock" : "DB"}
      </span>
          ...
          <section className="mt-6">
            ...
            <div className="bg-white shadow rounded-xl p-3 max-h-60 overflow-y-auto text-xs">
              {visibleFaults.map(f => (
                <div
                key={f.id}
                className="border border-red-200 bg-red-50 rounded-lg px-3 py-2 mb-2 last:mb-0 flex justify-between cursor-pointer hover:bg-red-100 active:bg-red-200"
                onClick={() =>
                  setVfdSelection({
                    controllerCode: f.controllerCode,
